<template>
        ...
        <div class="col-6 my-3" v-for="product in productsData" :key="'productList' + product.id">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <p class="card-text">
                <span class="badge bg-secondary bg-opacity-75 text-body-emphasis me-2">{{
                  product.category
                }}</span>
                <span class="badge bg-secondary bg-opacity-75 text-body-emphasis me-2">{{
                  product.areaCategory
                }}</span>
                <span class="badge bg-secondary bg-opacity-75 text-body-emphasis me-2">{{
  ...
  <!-- 元件：分頁 -->
  <Pagination ref="pagination" :pages="page" :get-per-page-data="getProductsData"> </Pagination>
</template>
...
export default {
  ...
  methods: {
    ...
    checkCartItemQty(id) {
      // 防呆：超過預設數量10
      const itemSelectedIndex = this.cartData.findIndex((item) => item.product.id === id)
